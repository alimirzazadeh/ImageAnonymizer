...
def test(parser, visualisation=None):
    ...
    checkPointDir = kwargs["dir"] + '/' + name
    print(checkPointDir)

    from ipdb import set_trace as bp 

    # bp()
    # hub_model = torch.hub.load(checkPointDir,'PGAN',config = None,useGPU = False,pretrained=False)
    # hub_model = torch.hub.load('facebookresearch/pytorch_gan_zoo:master', 'PGAN', config = None, useGPU = False, pretrained=False) # (Not pretrained models online yet)
    # model = PGAN(pretrained=True, model_name="celeba")
    ...
    imgOutput = img[0]
    # bp()
    m = nn.Upsample((input_image_shape[0],input_image_shape[1]))

    save_image(m(img), path)
